...
def delete_record():
    ...
    try:
        ...
        # Delete the record from the database based on the student number
        query = f"DELETE FROM grading_system WHERE Student_Number = '{student_number}'"
        cur.execute(query)
        con.commit()
        ...
        print("Delete record failed: {}".format(error))
    finally:
        if con.is_connected():
            con.close()
            print("MySQL Connection is now CLOSED")
...
label_title = ctk.CTkLabel(master=title_frame, text="SCORE SENSEI", font=("Roboto", 50, "bold"))
...
button_delete = ctk.CTkButton(master=main_frame, text="Delete Record", command=delete_record)
